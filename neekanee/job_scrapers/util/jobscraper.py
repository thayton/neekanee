...
from neekanee_browser import NeekaneeBrowser
...
class JobScraper(object):
    def __init__(self, company_dict=None, return_usa_only=True):
        self.init_browser()
        self.init_logger()
        self.init_geocoder()
        self.geocoder.return_usa_only = return_usa_only
    ...
    def init_logger(self):
        self.logger = logging.getLogger('neekanee.JobScraper')
    ...
    def prune_unlisted_jobs(self, listed_jobs, use_job_cmp=False):
        ...
        if use_job_cmp:
            ...
            for entry in jobs_to_delete:
                job = self.company.job_set.get(url=entry[0], url_data=entry[1])
                self.logger.debug('Deleting job (%s) for company %s' % (job.url, self.company))
                job.delete()
                num_deleted += 1
