import re, urlparse

from neekanee.jobscrapers.jobscraper import JobScraper
from neekanee.htmlparse.soupify import soupify, get_all_text
from neekanee.urlutil import url_query_filter

from neekanee_solr.models import *

COMPANY = {
    'name': 'Brookhaven National Laboratory',
    'hq': 'Upton, NY',

    'home_page_url': 'http://www.bnl.gov',
    'jobs_page_url': 'https://jobs.bnl.gov',

    'empcnt': [1001,5000]
}

class BnlJobScraper(JobScraper):
    def __init__(self):
        super(BnlJobScraper, self).__init__(COMPANY)

    def scrape_job_links(self, url):
        jobs = []

        self.br.open(url)

        s = soupify(self.br.response().read())
        r = re.compile(r'^POSTINGTITLE\$\d+$')
        
        for a in s.findAll('a', id=r):
            tr = a.findParent('tr')
            sp = tr.find('span', id=re.compile(r'^JOBNUMBER\$\d+$'))

            q = 'Page=HRS_CE_JOB_DTL&JobOpeningId=%d' % int(sp.text)
            u = urlparse.urlparse(self.br.geturl())
            u = list(u)
            u[4] = q
            u = urlparse.urlunparse(u)

            job = Job(company=self.company)
            job.title = a.text
            job.url = u
            job.location = self.company.location
            jobs.append(job)

        return jobs

    def scrape_jobs(self):
        job_list = self.scrape_job_links(self.company.jobs_page_url)
        self.prune_unlisted_jobs(job_list)
        new_jobs = self.new_job_listings(job_list)

        for job in new_jobs:
            self.br.open(job.url)

            s = soupify(self.br.response().read())
            d = s.find('div', id='PAGECONTAINER')

            job.desc = get_all_text(d)
            job.save()

def get_scraper():
    return BnlJobScraper()

if __name__ == '__main__':
    job_scraper = get_scraper()
    job_scraper.scrape_jobs()
