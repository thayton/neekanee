...
COMPANY = {
    ...
    'home_page_url': 'http://www.pstcc.edu/',
    'jobs_page_url': 'https://jobs.tbr.edu',

    'empcnt': [501,1000]
}

class PellissippiStateJobScraper(PeopleAdminJobScraper):
    def __init__(self):
        super(PellissippiStateJobScraper, self).__init__(COMPANY)
    ...
    def update_jobSearch_form(self):
        ...
        ctl.get(label='Pellissippi State Community College').selected = True

    def submit_jobSearch_form(self):
        self.br.submit()

def get_scraper():
    return PellissippiStateJobScraper()
